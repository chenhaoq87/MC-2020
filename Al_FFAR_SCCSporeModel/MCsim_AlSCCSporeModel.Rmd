---
title: "MCsim_AlSCCSporeModel"
author: "Sarah I. Murphy"
date: "12/23/2020"
output: pdf_document
---

This is a simplified version (deleted unnecessary stuff) of MCsim.Rmd found in MC-2020 on GitHub. Used model version used in "MCsim_RachelFISEModel.R" as basis. In this version, different input data are used for spore count, running the model with 3 different input data to then compare results; the input data for spore count will be prepared from the file "RawMilkSCC_vs_ColdTolerantSpores_Cleaned_122220.csv" (cleaned by Sarah from "RawMilkSCC_vs_ColdTolerantSpores_21122020.csv"), which Al had first prepared by "going through Steph’s farm project data and extracting data on cold-tolerant sporeformers in raw milk and somatic-cell count", which is a new file rather than what has been used previously in other uses of the spore model "CrossSectional_RawMPN_3.6.17.csv". 

The three subsets to be prepared from the original file "RawMilkSCC_vs_ColdTolerantSpores_21122020.csv" will be as follows: 
(i)  Full original dataset 
(ii) Subset with only data from the 10 farms with the lowest SCC. 
(iii) Subset with only data remaining after you remove the 10 farms with the highest SCC. 
...
Revised 12/23/2020, per Al's request, Sarah wrote code & ran for v4 where exclude farms w/ greater than 400,000 SCC
...
# (c) initial microbial count data (see notes above about this dataset that Al prepared; RawMilkSCC_vs_ColdTolerantSpores_21122020.csv)
# spore
# spore_init_file <- read.csv("InputFiles/CrossSectional_RawMPN_3.6.17.csv") #what used previously
spore_init_file <- read.csv("InputFiles/RawMilkSCC_vs_ColdTolerantSpores_Cleaned_122220.csv")

# prepare datasets per Al's instructions, except select or remove 10 farms instead of 10%

spore_init_file2 <- spore_init_file[spore_init_file$SCC %in%  head(sort(spore_init_file$SCC),10),] #lowest 10 SCC only

spore_init_file3 <- spore_init_file[spore_init_file$SCC %in% head(sort(spore_init_file$SCC),82),] #remove highest 10 SCC (result = lowest 82)

spore_init_file4 <- spore_init_file[spore_init_file$SCC <= 400000,] #exclude farms w/ SCC > 400,000 
...
```{r}
## (a) Initial contamination: logMPN normal distribution
# spore_init_file <- spore_init_file2
# spore_init_file <- spore_init_file3
spore_init_file <- spore_init_file4
...
#assume normal distribution for all 3 subsets, but that seems to be a bad assumption especially for (ii) aka data from spore_init_file2
...
## save results 
# data_v1 <- data2 #original dataset
# save(data_v1, file = "data_v1.RData")

# data_v2 <- data2 
# save(data_v2, file = "data_v2.RData") #lowest 10 SCC only

# data_v3 <- data2 #
# save(data_v3, file = "data_v3.RData") #remove highest 10 SCC (result = lowest 82)

data_v4 <- data2
save(data_v4, file= "data_v4.RData") #exclude farms with SCC > 400,000 per EU reg
```
