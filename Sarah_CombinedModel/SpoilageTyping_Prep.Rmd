...
```{r}
library(readxl)
df <- read_excel("VSL_Jul18-May19_Micro.xlsx") 
```
...
# dfw$Plant_VSLNumber_SampleID <- paste(dfw$Plant,dfw$VSLNumber_SampleID,sep="_") #to do after get updated dataset from Tim
dfw$countability[is.na(dfw$countability)==TRUE] <- "OK"
dfw <- dfw %>%
  filter(countability!="M") #need to doublecheck what M stands for; assume all others are OK to include even if "E" (estimated) or "S" (spreading?)

# # convert dates to appropriate format (to do after get updated dataset from Tim)
...
##START HERE, handle LOD and then prepare for expanding dataset and then can do the filtering stuff
dfw2<-dfw[c(12,13,6,14)]
