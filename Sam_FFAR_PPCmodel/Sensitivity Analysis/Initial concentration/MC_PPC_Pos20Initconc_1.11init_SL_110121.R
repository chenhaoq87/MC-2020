...
sink(file = "Pos20Initconcfilecatstatements_1.11init.txt")
...
sensanalysis <- function(log_count,percent){
  t <- log_count + abs(log_count)*percent
}

data$logMPN_init <- sensanalysis(data$logMPN_init,0.2) #increase initial conc 
...
save.image(file = "Pos20Initconc_ppcmodel_1.11init.RData")
