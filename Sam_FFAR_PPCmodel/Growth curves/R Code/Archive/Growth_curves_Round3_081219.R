#Growth curves ROUND 3
#written by MDP on 7/26/2019
#edited by SL on 8/12/2019
...
#get the data
growthmodel2 <- read.csv("Growth_curve_round1and3RAWDATAcombined_SL_080619.csv", stringsAsFactors = FALSE)
...
for (iso in isolate_list) {
 ...
 # for (i in seq(1, 3)){
    ...
    #draw the plot
    plot(growth_data$t, growth_data$LOG10N, xlim = c(0,240), ylim = c(3,9), xaxt='n', xlab = "Time (hours)", ylab = "LOG10N", pch = 20)
...
write.table(output, file = "Growth_Params_Round3_SL_081219.csv", sep=",", row.names=FALSE)
