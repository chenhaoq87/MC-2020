...
# (a) frequency data
# spore
spore_frequency_file <- "Ariel_2017MC_SporeModel/InputFiles/ColdgrowATFreq_CrossSect.csv"
spore_freq_import <- read.csv(spore_frequency_file, stringsAsFactors = FALSE, header = TRUE)
# ppc
ppc_frequency_file <- "Sam_FFAR_PPCmodel/InputFiles/Frequency_ONLYSAMISOLATES_remove13and23_020120.csv"
ppc_freq_import <- read.csv(ppc_frequency_file, stringsAsFactors = FALSE, header = TRUE)
...
names(ppc_freq_import)[names(ppc_freq_import) == 'X16S_ST'] <- 'RealAT'
ppc_freq_import$ClosestAT <- ppc_freq_import$RealAT
ppc_freq_import <- ppc_freq_import[c(5:6)]
...
hist(MPNdata$logMPN,freq=F,xlim = c(-4,4))
curve(dnorm(x,mean=fit$estimate[1],sd = fit$estimate[2]),add=T)

logMPN_mean <- fit$estimate[1]
logMPN_sd <- fit$estimate[2]
