...
```{r}
df8_14 <- d14 %>%
  filter(scenario=="NB_BestPPCVSL436_CT") %>%
  group_by(lot_id,day)%>%
  summarize(count=mean(count))
hist(df8_14$count, main="Histogram of simulated milk spore count over shelf-life\n Day = 14",  xlab=expression('Log'['10']*'MPN/mL'), xlim=c(-4,8), ylim=c(0,10),breaks=20, col="gray")

# df7$count<- as.numeric(df7$count)
# df7_14 <- df7 %>%
#   filter(day=="14")
# hist(df7_14$count, main="Histogram of simulated milk spore count over shelf-life\n Day = 14",  xlab=expression('Log'['10']*'MPN/mL'), xlim=c(-4,8), ylim=c(0,500),breaks=20, col="gray")
```
