...
# spore
spore_MPNdata <- read.csv("Ariel_2017MC_SporeModel/InputFiles/CrossSectional_RawMPN_3.6.17.csv")
# ppc
ppc_init_file <- read_csv("Sam_FFAR_PPCmodel/InputFiles/InitialCount_25percent_SL_101619.csv")
...
ppc_init_file <- ppc_init_file %>%
  .[c(1,3)]%>% #don't include initial micro count since error; asked Sam to check it
  rename(log10CFU = `LOG Initial microbial count`)
ppc_init_file$CFU <- 10^(ppc_init_file$log10CFU) #get CFU data
```
...
# (i) spore data
spore_MPNdata$logleft <- log10(spore_MPNdata$left)
spore_MPNdata$logright <- log10(spore_MPNdata$right)
spore_MPNdata$logMPN <- log10(spore_MPNdata$MPN)
cens_data <- spore_MPNdata[,c("logleft","logright")]
names(cens_data) <- c("left","right")
spore_fit <- fitdistcens(censdata = cens_data,distr = "norm")
hist(spore_MPNdata$logMPN,freq=F,xlim = c(-4,4))
curve(dnorm(x,mean=spore_fit$estimate[1],sd = spore_fit$estimate[2]),add=T)

spore_logMPN_mean <- spore_fit$estimate[1]
spore_logMPN_sd <- spore_fit$estimate[2]

# (ii) ppc data; https://stats.stackexchange.com/questions/132652/how-to-determine-which-distribution-fits-my-data-best
ppc_fit <- fitdist(data = ppc_init_file$CFU,distr = "norm")
ppc_fit <- fitdist(data = ppc_init_file$log10CFU,distr = "norm")
descdist(data = ppc_init_file$CFU)
plot(ppc_fit)
# initialcount_data <- initialcount_import[,3]
# initialcountlog_data <- initialcount_import[,4]

# OR instead of using data, instead you can just set mean and sd to use for normal distribution:
