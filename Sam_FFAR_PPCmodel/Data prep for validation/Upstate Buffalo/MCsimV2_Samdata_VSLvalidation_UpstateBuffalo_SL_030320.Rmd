...
init_file <- "Initialmicro_upstatebuffalo_MCinput_030220.csv"
```
...
+ Troubleshooting
  + Frequency of only plant
```{r}
#input files
#frequency_file <-"Sam_FFAR_PPCmodel/Data prep for validation/Upstate Buffalo/Frequency_UpstateBuffalo_030420.csv"
frequency_file <-"freq_16S_ppc.csv"
growth_file <- "ST_percentidentity_forcedNmax_MCinput_SL_030320.csv"
init_file <- "Initialmicro_upstatebuffalo_MCinput_030220.csv"
```
...
freq_import <- freq_import %>%
  filter(X16S.ST!=42)
# freq_data = freq_import$X16S_ST
freq_data = freq_import$X16S.ST
...
ppc_fit <- fitdist(data = initialcount_import$LOG.initial,distr = "norm") #TO DO: find the best fitting distribution; not normal 
...
logMPN_mean <- c(0.4191862) #day initial
logMPN_sd <- c(0.8497558) #day initial
...
for (i in 1:(n_sim *n_halfgal * n_day)){
  ...
  data$count[i] <- log10N(data$day[i]*24, growth_import$lag[allele_index], growth_import$mu[allele_index],data$logMPN_init[i],growth_import$Nmax[allele_index],growth_import$modelname[allele_index])
...
day7andday10<-read.csv("LOGday7and10_upstatebuffalo_030220.csv")
