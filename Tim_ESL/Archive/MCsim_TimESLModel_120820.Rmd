...
* constant temperature for duration of the transportation
* duration of transportation is the same for each unit by lot; Tim to find data on this/appropropriate distribution
* temperature is the same for each unit by lot for transportation from facility to store; Tim to find data on this/appropropriate distribution
```{r}
## (a)  Sample the temperature distribution & add to df
temps_T2 <- rep(rtruncnorm(n_sim,a=-1.4,b=5.4,mean=2.3,sd=1.8),each=n_units) #triangular distribution
data$T_T2 <- temps_T2

## (b) Sample the storage time (in days) distribution & add to df
times_T2 <- rep(rtruncnorm(n_sim,a=0.042,b=10.0, mean=1.821,sd=3.3),each=n_units)
data$t_T2 <- times_T2
...
* temperature is different for each unit for storage at home; Tim to select data appropropriate distribution (confirm Laplace?)
```{r}
