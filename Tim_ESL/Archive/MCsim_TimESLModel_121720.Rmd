...
- based on data from temperature evaluation, determine "mid-point" of the temperatures by times; use time from distribution ("duration of transportation" above) to the determine the change in temperature to use for Stage 4; additionally, include "variability" to this based on the data from 1999 temp eval assessment for <70F, 70F, and >70F, which range from -X% to +X%
```{r}
...
#b. Assumes shift in temperature does not require an adjustment period
#λshift = (1 + γ – φ) * λT , where γ = 0, then  λshift = (1 – φ) * λT  
data$adjLag_T2 <- (1 - (data$t_F/data$newLag_F)+(data$t_T/data$Lag_T)+(data$t_S/data$Lag_S))*data$newLag_S 
...
#make a new dataframe for modeling home storage
start_day <- 1 #First time point (in days) in unit's shelf life to simulate
end_day <- 35 #Last time point (in days) in unit's shelf life to simulate

#get needed data from "data"
df2 <- data[c(1:3,14,15,17)] #lot_id, unit_id, STorAT, T_T2, count_T2, T_H; will also need info for lag, but ignore for now?

##

# #now determine count
# for (i in 1:(n_sim *n_units * end_day)){
#   #Find row in growth parameter data that corresponds to allele sample
