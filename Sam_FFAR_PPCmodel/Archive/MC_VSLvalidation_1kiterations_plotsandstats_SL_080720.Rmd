...
## Script purpose: Validation of the 10k simulations
##################################################
#Edited by SL on 07-14-20
##################################################
```
...
```{r}
load("Z:/sl2763_Samantha Lau/Projects/2019 FFAR project/Data prep for validation/Model/10ksimulations/Data_10kiterations_truncated_VALIDATION6C.RData")
```
...
hist(day7counts_sim, 10, col="black", main="Day 7 simulated histogram", 
     xlab="LOG CFU/mL", xlim=c(-2,10), ylim = c(0,0.8), freq = FALSE)
...
plot(density(na.omit(day7counts_sim)), main="Day 7 simulated histogram", 
     xlab="LOG CFU/mL", xlim=c(-2,10), ylim=c(0,0.8))
```
...
+ Calculate proportion of samples that have spoiled WHEN Remove ST 13, the ST with the highest growth rate by setting final conc as 0
```{r}
...
+ Calculate proportion of samples that have spoiled WHEN REMOVING ST WITH HIGHEST GROWTH RATE
  + Set count to desired log value
  + Ex: If I want to see proportions of samples that reach 1,000,000 CFU/mL, set count>6
  + Ex: If I wanted to see proportions of samples that reach 20,000 CFU/mL, set count>4.3
  + Proportion= samples that reach X log for Y day/ total samples for Y day
...
+ Calculate proportion of samples that have spoiled WHEN REMOVING 2 MOST FREQUENT ST
  + Set count to desired log value
  + Ex: If I want to see proportions of samples that reach 1,000,000 CFU/mL, set count>6
  + Ex: If I wanted to see proportions of samples that reach 20,000 CFU/mL, set count>4.3
  + Proportion= samples that reach X log for Y day/ total samples for Y day
