...
# data$logCount_init  <- 0.549 #change for each of 5 rounds here & in next chunk
```
...
for (i in 1:(n_sim *n_units * n_day)){
  ...
  data$count[i] <- log10N_func(data$day[i],spore_growth_import$lag[allele_index], spore_growth_import$mumax[allele_index], -4.279, spore_growth_import$LOG10Nmax[allele_index])}
```
## 7. Save results
...
save(data,file="MCsim_result_df_new1.RData")
