...
sink(file = "Microbialcoatingfilecatstatements_1.11init.txt")
...
#want to arrange data based on first two columns
data <- data %>% arrange(BT, half_gal)
#assign a unique value based on the first two columns 
# ( I want the samples that come from the same bulk tank and half gallon to have the same ID)
data$ID <- cumsum(!duplicated(data[1:2]))

#assign 10% of those the sample IDs with starting concentration -2log

#pick a set of random numbers from 1-10000
#(because there are 10,000 unique IDs and 10% of 10,000 would be 1000)
selectsamples<-sample.int(100000, 10000, replace = FALSE)

for (i in selectsamples){
  data$logMPN_init <- ifelse(data$ID== i, data$logMPN_init-2 , data$logMPN_init )
}

#ifelse(data$ID %in% selectsamples, data$logMPN_init2 <- data$logMPN_init-2 , data$logMPN_init2<- data$logMPN_init)


data0<- data[,c(1,2,3,4,5,6)]
...
data0$BT_hg <- paste(data0$BT,data0$half_gal,sep="_")
...
data2 <- merge(df_temp,data0,by="BT_hg")
####
...
for (i in 1:(n_sim *n_halfgal)){
  ...
  #an adjustment factor of 0.684 was calculated by comparing the average growth rate of the isolates done by Sam L
  #and the growth rate from VSL data. The growth rate was extrapolated from day 7 and day 10 VSL data. 
  #The growth curves done by Sam L had a higher growth rate so to get it to equal to what is seen in VSL, we multiplied
  #the growth curve growth rates by 0.684. 
  newMu<-newMu*0.684
...
save.image(file = "Microbialcoating_ppcmodel_1.11init.RData")
