...
for (i in 1:(n_sim *n_units * n_day)){
  ...
  prev_row <- getPrevRow(data, row$lot_id, row$milk_unit, row$day)
  update <- ifelse(nrow(prev_row) == 0 || row$temp != prev_row$temp, T, F)
  data$count[i] <- log10N_func(data$day[i],spore_growth_import$lag[allele_index],spore_growth_import$mumax[allele_index],data$logCount_init[i],spore_growth_import$LOG10Nmax[allele_index])}
