...
```{r}
growth_file <- "ST_medianmuandlag_MCinput_SL_021420.csv"
...
init_file <- "Initialmicrodata_MCinput_SL_021420.csv"
```
...
logMPN_mean <- c(0.0704966) #day initial
logMPN_sd <- c(1.27629) #day initial
...
day7andday10<-read.csv("Log microbial count__MCinput_021419.csv")
...
plot(density(na.omit(day10counts_sim)), main="Day 10 simulated vs VSL", 
     xlab="LOG CFU/mL", xlim=c(0,10), ylim=c(0,1.2))
...
plot(density(na.omit(newlogday10)), main="Day 7 vs 10 VSL/Simulated histogram", 
     xlab="LOG CFU/mL", xlim=c(-1,10), ylim=c(0,1.0))
...
q2=ecdf(newlogday10)
plot(q, verticals = TRUE, do.points=FALSE)
