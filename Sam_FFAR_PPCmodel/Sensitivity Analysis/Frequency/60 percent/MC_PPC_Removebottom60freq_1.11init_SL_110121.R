...
sink(file = "Removebottom60filecatstatements_1.11init.txt")
...
##remove the top20% based on freq fast growers
freq_import2<-freq_import
top20<-read.csv("Top60slowestST_SL_032721.csv")
colnames(top20)[1] = "X16S_ST"
newdf <- freq_import2[!(freq_import2$X16S_ST %in% top20$X16S_ST),]
freq_data = newdf$X16S_ST
freq_vec<-as.vector(freq_data)
...
save.image(file = "Removebottom60_ppcmodel_1.11init.RData")
