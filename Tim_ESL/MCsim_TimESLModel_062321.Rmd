...
Revised 02/03/21, completed the first and third validation. Empirically determined the T0 value during the third validation by calculating the RMSE for mean, median, sd and D-value. THe final value is 1.15 C. Also performed the sensitivity analysis using PRCC and proposed 3 what-if scenarios. 
...
source("Validation/Third validation.R", print.eval = T)
...
sum(df3$count[sim_day==21]>log10(20000))/length(df3$count[sim_day==21])
...
sa_result = sa$gamma
names(sa_result) = names(dat2)[-ncol(dat2)]
sa_result = sort(sa_result)
barplot(sa_result, horiz = TRUE, names.arg = names(sa_result), las=1,cex.names = 0.5)
...
spoiled = as.factor(dat[,ncol(dat)]>=log10(20000))
cf.fit = train(x = dat[,-ncol(dat)],
                y = spoiled,
                method = "cforest",
                metric = "Accuracy",
