...
growth_file <- "ST_percentidentity_forcedNmax_MCinput_SL_030320.csv"
init_file <- "Initialmicro_cornelldairy_MCinput_030220.csv"
```
...
+ Troubleshooting
  + Frequency of plant only
```{r}
#input files
frequency_file <- "Frequency_CornellDairy_030420.csv"
growth_file <- "ST_percentidentity_forcedNmax_MCinput_SL_030320.csv"
init_file <- "Initialmicro_cornelldairy_MCinput_030220.csv"
```
...
logMPN_mean <- c(1.508703) #day initial
logMPN_sd <- c(0.5901045) #day initial
...
day7andday10<-read.csv("LOGday7and10_cornelldairy_030220.csv")
...
```{r}
#VSL data for day 14 and 21
day7andday10<-read.csv("LOGday7and10_cornelldairy_030220.csv")

#create a histogram/distribution just for the VSL data 
logday14<-day7andday10$LOG.D14
newlogday14<-as.numeric(logday14) #use the log day 14 column from VSL data
logday21<-day7andday10$LOG.D21
newlogday21<-as.numeric(logday21) #use the log day 21 column from VSL data

#histogram with proportions for day 14
hist(newlogday14, 10, col="black", main="Day 14 VSL histogram", 
     xlab="LOG CFU/mL", xlim=c(0,8), ylim = c(0,0.8), freq = FALSE)
lines(density(na.omit(newlogday14)))

# plot density plot for day 14
plot(density(na.omit(newlogday14)), main="Day 14 VSL histogram", 
     xlab="LOG CFU/mL", xlim=c(0,8), ylim=c(0,0.8))

#histogram with proportions for day 21
hist(newlogday21, 10, col="black", main="Day 21 VSL histogram", 
     xlab="LOG CFU/mL", xlim=c(0,10), ylim = c(0,0.8), freq = FALSE)
lines(density(na.omit(newlogday21)))

# plot density plot for day 21
plot(density(na.omit(newlogday21)), main="Day 21 VSL histogram", 
     xlab="LOG CFU/mL", xlim=c(0,10), ylim=c(0,0.8))

shapiro.test(newlogday14)
qqnorm(newlogday14)
shapiro.test(newlogday21)
qqnorm(newlogday21)
...
+ Simulating data for day 14
```{r}
#Make df of day 14 only 
#Note: change this to day==4 when the initial mean and SD is set using the day 7 data instead of initial
day14counts<- data2_counts[data2_counts$day=="14",] 

#only using the count data
day14counts_sim<-day14counts$count

#making a numeric factor
day14counts_sim<-as.numeric(day14counts_sim)

#day14factor
day14countfactor<-as.factor(day14counts_sim)
```

+ Comparing simulated and VSL data for day 14
```{r}
#histogram with simulated day 14
hist(day14counts_sim, 10, col="black", main="Day 14 simulated histogram", 
     xlab="LOG CFU/mL", xlim=c(-2,12), ylim = c(0,0.8), freq = FALSE)
lines(density(na.omit(day14counts_sim)))

#plot with day 14 simulated histogram
plot(density(na.omit(day14counts_sim)), main="Day 14 simulated histogram", 
     xlab="LOG CFU/mL", xlim=c(1,12), ylim=c(0,0.8))

#create a histogram with both the simulated and the VSL data
plot(density(na.omit(day14counts_sim)), main="Day 14 simulated vs VSL", 
     xlab="LOG CFU/mL", xlim=c(-2,12), ylim=c(0,1.2))
lines(density(na.omit(newlogday14)), col="blue")
legend('topright',  legend = c("simulated", "VSL"), 
       lty=1, col=c('black', 'blue'), bty='n', cex=.75)


#make ECDF for day 14
q=ecdf(day14counts_sim)
q2=ecdf(newlogday14)
plot(q, verticals = TRUE, do.points=FALSE,  xlab="LOG10 CFU/mL", main="Day 14 simulated vs VSL", ylab = "Density", xlim=c(-2,10))
plot(q2, verticals = TRUE, do.points=FALSE, add=TRUE, col="blue")
legend('bottomright',  legend = c("simulated", "VSL"), 
       lty=1, col=c('black', 'blue'), bty='n', cex=.75)
```
...
+ Running Kolmogorov-Smirnov test for day 14
  + Purpose: Want to compare the distributions of the VSL versus simulated
```{r}
ks.test(day14counts_sim, newlogday14, alternative = c("two.sided"), exact = NULL)


#calculating critical value
x <- day14counts_sim[!is.na(day14counts_sim)]
n <- length(x)
  y <- newlogday14[!is.na(newlogday14)]
  n.y <- length(y)

a=1.36
day14critvalue<-1.36*((n+n.y)/(n*n.y))^0.5
day14critvalue

#5%level significance
a=1.35810
day14critvalue5<-1.35810*((n+n.y)/(n*n.y))^0.5
day14critvalue5

#10%level significance
a=1.22385
day14critvalue10<-1.22385*((n+n.y)/(n*n.y))^0.5
day14critvalue10


#20%level significance
a=1.07275
day14critvalue20<-1.07275*((n+n.y)/(n*n.y))^0.5
day14critvalue20
...
+ Simulating data for day 21
```{r}
#Make df of day 21 only 
#Note: change this to day==4 when the initial mean and SD is set using the day 7 data instead of initial
day21counts<- data2_counts[data2_counts$day=="21",] 

#only using the count data
day21counts_sim<-day21counts$count

#making a numeric factor
day21counts_sim<-as.numeric(day21counts_sim)

#day21factor
day21countfactor<-as.factor(day21counts_sim)
```

+ Comparing simulated and VSL data for day 21
```{r}
#histogram with simulated day 10
hist(day21counts_sim, 10, col="black", main="Day 21 simulated histogram", 
     xlab="LOG CFU/mL", xlim=c(-2,12), ylim = c(0,0.8), freq = FALSE)
lines(density(na.omit(day21counts_sim)))

#plot with day 21 simulated histogram
plot(density(na.omit(day21counts_sim)), main="Day 21 simulated histogram", 
     xlab="LOG CFU/mL", xlim=c(1,12), ylim=c(0,0.8))

#create a histogram with both the simulated and the VSL data
plot(density(na.omit(day21counts_sim)), main="Day 21 simulated vs VSL", 
     xlab="LOG CFU/mL", xlim=c(-2,12), ylim=c(0,1.2))
lines(density(na.omit(newlogday10)), col="blue")
legend('topright',  legend = c("simulated", "VSL"), 
       lty=1, col=c('black', 'blue'), bty='n', cex=.75)
...
#make ECDF for day 21
q=ecdf(day21counts_sim)
q2=ecdf(newlogday21)
plot(q, verticals = TRUE, do.points=FALSE,  xlab="LOG10 CFU/mL", main="Day 21 simulated vs VSL", ylab = "Density", xlim=c(-2,10))
plot(q2, verticals = TRUE, do.points=FALSE, add=TRUE, col="blue")
legend('bottomright',  legend = c("simulated", "VSL"), 
       lty=1, col=c('black', 'blue'), bty='n', cex=.75)
```

+ Running Kolmogorov-Smirnov test for day 21
  + Purpose: Want to compare the distributions of the VSL versus simulated
```{r}
ks.test(day21counts_sim, newlogday21, alternative = c("two.sided"), exact = NULL)


#calculating critical value
x <- day21counts_sim[!is.na(day21counts_sim)]
n <- length(x)
  y <- newlogday10[!is.na(newlogday21)]
  n.y <- length(y)

a=1.36
day21critvalue<-1.36*((n+n.y)/(n*n.y))^0.5
day21critvalue

#5%level significance
a=1.35810
day21critvalue5<-1.35810*((n+n.y)/(n*n.y))^0.5
day21critvalue5

#10%level significance
a=1.22385
day21critvalue10<-1.22385*((n+n.y)/(n*n.y))^0.5
day21critvalue10


#20%level significance
a=1.07275
day21critvalue20<-1.07275*((n+n.y)/(n*n.y))^0.5
day21critvalue20
...
+ Comparing day 14 and day 21 concentrations for VSL and simulated
```{r}
# plot density plot of day 14 and day 21
plot(density(na.omit(newlogday14)), main="Day 14 vs 21 VSL/Simulated histogram", 
     xlab="LOG CFU/mL", xlim=c(-2,12), ylim=c(0,1.0),col="red")
lines(density(na.omit(newlogday21)), col="black", lty=1) #VSL day 21 data
lines(density(na.omit(day14counts_sim)), col="black", lty=2) #simulated day 21 data
lines(density(na.omit(day21counts_sim)), col="red", lty=2) #simulated day 14 data
legend('topright',  legend = c("14 VSL", "21 VSL", "14 sim", "21 sim"), 
       lty=c(1,1,2,2), col=c('red', 'black','red', 'black'), bty='n', cex=.55)
```
